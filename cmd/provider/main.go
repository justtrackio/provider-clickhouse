...
import (
	...
	apisCluster "github.com/justtrackio/provider-clickhouse/apis/cluster"
	apisNamespaced "github.com/justtrackio/provider-clickhouse/apis/namespaced"
	"github.com/justtrackio/provider-clickhouse/config"
	"github.com/justtrackio/provider-clickhouse/internal/clients"
	controllerCluster "github.com/justtrackio/provider-clickhouse/internal/controller/cluster"
	controllerNamespaced "github.com/justtrackio/provider-clickhouse/internal/controller/namespaced"
	"github.com/justtrackio/provider-clickhouse/internal/features"
	"github.com/justtrackio/provider-clickhouse/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for ClickHouse").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-clickhouse"))
	if *debug {
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped ClickHouse APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced ClickHouse APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped ClickHouse controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced ClickHouse controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped ClickHouse controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced ClickHouse controllers")
	}
